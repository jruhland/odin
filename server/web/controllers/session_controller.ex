defmodule Odin.SessionController do
  use Odin.Web, :controller

  require Logger

  def create(conn, %{"grant_type" => "password", "username" => username, "password" => password}) do
    try do
      # Attempt to retrieve exactly one user from the DB, whose email matches the one provided
      ...
      cond do
        ...
        true ->
          ...
          conn
          |> put_status(401)
          |> render(Odin.ErrorView, "401.json") # 401
      end
    rescue
      e ->
